...
const Header = () => {
    ...
    const isAuth = useSelector(state => state.userAuth.userAuth.token)
    ...
    const userSignOut = () =>{
        const auth = getAuth()
        signOut(auth).then(() => dispatch(userAuthAction({})))
            .catch((error) => console.log(error))
    }

    return (
        <div className={`${styles.headerWrapper} d-flex space-between align-center`}>
            <div className={`d-flex ${styles.headerLeft}`}>
                <Link to="/">
            ...
            <div className={`d-flex ${styles.headerRight}`}>
                {!!isAuth ? (
                    <>
                        <div className={styles.headerBasket} onClick={() => dispatch(showBasketAction(!showBasket))}>
                            ...
                            {totalPrice === 0 ? <p>(пусто)</p> : <p>{cost} руб.</p>}
                        </div>
                        ...
                        <div className={styles.headerOut}>
                            <FontAwesomeIcon onClick={userSignOut} icon={faSignOutAlt}/>
                        </div>
                    </>
                ) : (
                    <div className={styles.headerLogin}>
                        <div className="d-flex">
                            ...
                            <img src="/image/sign-in.png" alt="sine-in"/>
                        </div>
                        <div className="d-flex">
                            <Link to="/sign-up">Регистрация</Link>
                            <img src="/image/sign-up.png" alt="sine-up"/>
                        </div>
                    </div>
                )}
            </div>
